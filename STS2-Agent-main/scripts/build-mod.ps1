param(
    ...
    [string]$GodotExe = "",
    [switch]$AllowPackedPckOnNonZeroExit
)
...
function Test-CanAcceptNonZeroGodotExit {
    param(
    ...
    if (-not $AllowPackedPckOnNonZeroExit) {
        return $false
    }
    ...
    $isGameExe = [System.IO.Path]::GetFileName($ExecutablePath) -ieq "SlayTheSpire2.exe"
    $isKnownCrashExit = $ExitCode -eq -1073741819

    return $isGameExe -and $isKnownCrashExit
}
...
if ($LASTEXITCODE -ne 0) {
    if (Test-CanAcceptNonZeroGodotExit -ExecutablePath $GodotExe -OutputPath $pckOutput -ExitCode $LASTEXITCODE -AllowPackedPckOnNonZeroExit:$AllowPackedPckOnNonZeroExit) {
        Write-Warning "[build-mod] Godot exited with $LASTEXITCODE after producing a PCK. Continuing because the output file exists and the executable is the exported game runtime."
    } else {
